...
namespace Procon.Core.Test.Repositories {
    ...
    public class TestFlatPackedPackage {
        ...
        protected FlatPackedPackage SetupGetModifiedFilesFlatPackage(bool setInstalledVersion = true, bool setUpdatedVersion = true, bool setAvailableVersion = true) {
            ...
            if (setInstalledVersion == true) package.SetInstalledVersion(package.Versions.First(p => p.Version.SystemVersion == new Version(0, 0, 0, 1)));
            if (setUpdatedVersion == true) package.SetUpdatedVersion(package.Versions.First(p => p.Version.SystemVersion == new Version(1, 2, 3, 4)));
            if (setAvailableVersion == true) package.SetAvailableVersion(package.Versions.First(p => p.Version.SystemVersion == new Version(9, 9, 9, 9)));
        ...
        public void TestFlatPackedPackageCopying() {
            ...
            FlatPackedPackage originalCopiedTo = (FlatPackedPackage)original.Copy(package);
