...
namespace Procon.Database.Serialization.Test.SqLite {
    ...
    public class TestSerializerSqLiteFindExplicit : TestSerializerFind {
        ...
        public override void TestSelectAllFromPlayerWhereKdrGreaterThanEqualTo31F() {
            Assert.AreEqual(@"SELECT * FROM `Player` WHERE `Kdr` >= 3.1", new SerializerSqLite().Parse(this.TestSelectAllFromPlayerWhereKdrGreaterThanEqualTo31FExplicit).Compile().Compiled.First());
        }

        [Test]
        public override void TestSelectAllFromPlayerWhereKdrLessThanEqualTo31F() {
            Assert.AreEqual(@"SELECT * FROM `Player` WHERE `Kdr` <= 3.1", new SerializerSqLite().Parse(this.TestSelectAllFromPlayerWhereKdrLessThanEqualTo31FExplicit).Compile().Compiled.First());
        }
