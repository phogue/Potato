...
namespace Procon.Fuzzy {
    ...
    public class Phrase : List<Token> {
        ...
        public Phrase AddDistinctRange(IEnumerable<Token> collection) {
            var worseMatch = from token in this from newToken in collection where String.CompareOrdinal(token.ToString(), newToken.ToString()) == 0 && newToken.Similarity < token.Similarity select newToken;
            /*
            ...
            */
            ...
            collection.Where(x => worseMatch.Contains(x) == false).ToList().ForEach(this.Add);
