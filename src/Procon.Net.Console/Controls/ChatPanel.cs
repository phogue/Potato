    ...
    public partial class ChatPanel : UserControl {
        ...
        private void listCleanup_Elapsed(object sender, ElapsedEventArgs e) {
            ...
            var expiredList = this.lsvChatObjects
                ...
                .Cast<ListViewItem>()
                .Where(
                    x => ((x.Tag is ProtocolObject) && (x.Tag as ProtocolObject).Created < DateTime.Now.AddMinutes(-5))
                ).Select(x => x);
            ...
            if (e.EventType == GameEventType.Chat) {
                ...
                if (e.Chat != null) {
                    ...
                    ListViewItem item = new ListViewItem() {
                        Text = e.Chat.Created.ToShortTimeString(),
                        Tag = e.Chat
                    };
                    ...
                    item.SubItems.AddRange(
                        new ListViewItem.ListViewSubItem[] {
                            new ListViewItem.ListViewSubItem() {
                                Text = e.Chat.Origin.ToString()
                            },
                            new ListViewItem.ListViewSubItem() {
                                Text = e.Chat.Author.Name
                            },
                            new ListViewItem.ListViewSubItem() {
                                Text = e.Chat.Text
                            }
                    ...
                    if (e.Chat.Origin == ChatOrigin.Player) {
                        this.rtbChat.AppendText(String.Format("^0[{0}] ^2{1}: {2}{3}", e.Chat.Created.ToShortTimeString(), e.Chat.Author.Name, e.Chat.Text, Environment.NewLine));
                    }
                    else if (e.Chat.Origin == ChatOrigin.Reflected) {
                        this.rtbChat.AppendText(String.Format("^0[{0}]: ^4{1}{2}", e.Chat.Created.ToShortTimeString(), e.Chat.Text, Environment.NewLine));
                    }
            ...
            if (this.ActiveGame != null) {
                this.ActiveGame.Action(
                    new Chat() {
                        ...
                        Author = new Player(),
                        Text = this.txtQuickAnnounce.Text
                    }
                );
