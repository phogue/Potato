...
namespace Procon.UI
{
    ...
    internal static class Settings
    {
        ...
        {
            if (ExtensionApi.Settings.ContainsKey(key))
                return (T)ExtensionApi.Settings[key].Value;
            return fallback;
        }
        public static void Set(String key, Object value)
        {
            ExtensionApi.Settings[key].Value = value;
        }
                ...
                foreach (XElement nodeSetting in tNode.Elements()) {
                    if ((tAttribute  = nodeSetting.Attributes("type").FirstOrDefault()) != null &&
                        ...
                        if (tAttribType.IsPrimitive) {
                            ExtensionApi.Settings.Add(nodeSetting.Name.LocalName, Convert.ChangeType(nodeSetting.Value, tAttribType));
                        }
                        else {
                            ExtensionApi.Settings.Add(nodeSetting.Name.LocalName, TypeDescriptor.GetConverter(tAttribType).ConvertFrom(nodeSetting.Value));
                        }
                    }
                }
        ...
        public static void Save()
        {
            ...
            foreach (String key in ExtensionApi.Settings.Keys)
                if (ExtensionApi.Settings[key].Value != null)
                    tConfig.Root.Add(
                        new XElement(key,
                            new XAttribute("type",
                                (ExtensionApi.Settings[key].Value.GetType().Module.Name == "mscorlib.dll") ?
                                    ExtensionApi.Settings[key].Value.GetType().FullName :
                                    ExtensionApi.Settings[key].Value.GetType().AssemblyQualifiedName
                            ),
                            ExtensionApi.Settings[key].Value
                    ));
